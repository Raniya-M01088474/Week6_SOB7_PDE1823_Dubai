...
def switch_player(player):
    # Python requires == for checking equality. A single = is only used to assign a variable.
    if player == 1:
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        # Added the missing colon at the end of the while loop statement
        while not available:
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            
            # check_space_empty requires 3 arguments, so we had to pass 'column' to it as well
        ...
        # Uncommented this line! Without it, the game never checks if someone won and will always end in a Tie.
